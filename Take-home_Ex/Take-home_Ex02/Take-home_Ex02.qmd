...
```{r}
pacman::p_load(plyr , sf, tidyverse, readxl, tmap, maptools, kableExtra, sfdep, spdep, plotly)
```
...
\``{r} vaccine_rate_pivot <- vaccine_rate_pivot %>% left_join(jakarta_data, by=c("Sub_District" = "Sub_District")) vaccine_rate_pivot <- st_as_sf(vaccine_rate_pivot)`
...
```{r}
vaccine_rate_nb <- vaccine_rate_st %>%
  activate("geometry") %>%
